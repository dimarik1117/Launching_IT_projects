...
export default class Computer extends SimpleGameObject {
	...
	step(board, checkWin, simpleGameObject, view) {
		...
		// Приоритет хода.В зависимости от него компьютер обороняется/атакует 
		let priority = 0;
		// Индексы ячейки, которую нужно заполнить в зависимости от приоритета
		let px, py; 

		for (let i = 0; i <= count - 1; i++) {
			for (let j = 0; j <= count - 1; j++) {
				if (board[i][j] === "") {
					// Имитирует ход компьютера
					board[i][j] = this.icon;
					// Если ход компьютера победный, то сохраняем индексы ячейки
